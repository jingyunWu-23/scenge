    ...
    def update_params(self):
        ...
        for param in self.opt_params.keys():
            if len(self.opt_record[param]) > 1:
                ...
                self.opt_params[param].low = round(
                    max(mean - std, self.opt_params[param].min), 2
                )
                self.opt_params[param].high = round(
                    min(mean + std, self.opt_params[param].max), 2
                )
    ...
    def setSimulation(self, scene):
        ...
        try:
            ...
            self.simulation = self.simulator.createSimulation(
                scene, verbosity=self.args.verbosity
            )
