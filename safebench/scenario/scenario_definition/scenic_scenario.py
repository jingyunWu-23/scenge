...
class ScenicScenario:
    ...
    def _update_route_and_ego(self, timeout=None):
        ...
        for other_actor in self.world.scenic.simulation.objects:
            ...
            elif other_actor.carlaActor.id != ego_vehicle.id:
                env_actor = other_actor.carlaActor
                self.env_actors.append(env_actor)
                CarlaDataProvider._carla_actor_pool[env_actor.id] = env_actor
                CarlaDataProvider.register_actor(env_actor)
        ...
        self.timeout = (
            self._estimate_route_timeout(route) if timeout is None else timeout
        )
    ...
    def get_running_status(self, running_record):
        running_status = {
            ...
            "ego_pitch": CarlaDataProvider.get_transform(
                self.ego_vehicle
            ).rotation.pitch,
        ...
        env_actor_status = {}
        for i, env_actor in enumerate(self.env_actors):
            env_actor_status[f"env_actor_{i}"] = {
                "velocity": CarlaDataProvider.get_velocity(env_actor),
                "acceleration_x": env_actor.get_acceleration().x,
                "acceleration_y": env_actor.get_acceleration().y,
                "acceleration_z": env_actor.get_acceleration().z,
                "x": CarlaDataProvider.get_transform(env_actor).location.x,
                "y": CarlaDataProvider.get_transform(env_actor).location.y,
                "z": CarlaDataProvider.get_transform(env_actor).location.z,
                "roll": CarlaDataProvider.get_transform(env_actor).rotation.roll,
                "pitch": CarlaDataProvider.get_transform(env_actor).rotation.pitch,
                "yaw": CarlaDataProvider.get_transform(env_actor).rotation.yaw,
            }
        running_status.update(env_actor_status)
        ...
        for scenario in self.list_scenarios:
            ...
            if scenario.check_scenic_terminate():
                self.logger.log(
                    ">> Scenario stops due to scenic termination", color="yellow"
                )
                stop = True
                break
    ...
    def _create_criteria(self):
        ...
        criteria["collision"] = CollisionTest(
            actor=self.ego_vehicle, terminate_on_failure=True
        )
        ...
        if self.config.scenario_id != 0:  # only check when evaluating
            ...
            criteria["route_complete"] = RouteCompletionTest(
                self.ego_vehicle, route=route
            )
        return criteria
