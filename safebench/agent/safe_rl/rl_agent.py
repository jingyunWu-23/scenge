...
class RLAgent:
    ...
    def __init__(self, config, logger):
        ...
        self.model_file = (
            self.model_path
            if os.path.splitext(self.model_path)[1] in (".pt", ".torch")
            else None
        )
        model_dir = os.path.dirname(self.model_path) if self.model_file else self.model_path
    ...
    def load_model(self, episode=None, replay_buffer=None):
        if self.model_file is not None:
            if os.path.isfile(self.model_file):
                self.logger.log(f">> Loading {self.policy_name} model from {self.model_file}")
                self.policy.load_model(self.model_file)
            else:
                self.logger.log(
                    f">> No {self.policy_name} model found at {self.model_file}", "red"
                )
            return
    ...
    def save_model(self, episode, replay_buffer=None):
        filepath = self.model_file or os.path.join(
            self.model_path, f"model.sac.{episode:04}.torch"
        )
