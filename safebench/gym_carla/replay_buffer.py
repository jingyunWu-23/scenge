...
class RouteReplayBuffer:
    ...
    def load_buffer(self, path):
        ...
        self.collision_buffer_scenario_actions = data[
            "collision_buffer_scenario_actions"
        ]
    ...
    def store(self, data_list, additional_dict):
        ...
        for s_i in range(len(additional_dict)):
            ...
            if additional_dict[s_i]["collision"]:
                ...
                self.collision_buffer_scenario_actions[sid].append(
                    scenario_actions[s_i]
                )
                ...
                for key in additional_dict[s_i].keys():
                    ...
                    self.buffer_additional_dict[s_i][key].append(
                        additional_dict[s_i][key]
                    )
                self.buffer_len += 1
    ...
    def sample_init(self, batch_size):
        ...
        for key in self.buffer_init_additional_dict.keys():
            batch[key] = torch.cat(self.buffer_init_additional_dict[key][start_idx:])[
                sample_index
            ]
        return batch
    ...
    def sample(self, batch_size):
        ...
        for s_i in range(self.num_scenario):
            ...
            prepared_collision_scenario_actions += (
                self.collision_buffer_scenario_actions[s_i][collision_start_idx:]
            )
            prepared_collision_obs += self.collision_buffer_obs[s_i][
                collision_start_idx:
            ]
            prepared_collision_next_obs += self.collision_buffer_next_obs[s_i][
                collision_start_idx:
            ]
            prepared_collision_rewards += self.collision_buffer_rewards[s_i][
                collision_start_idx:
            ]
            prepared_collision_dones += self.collision_buffer_dones[s_i][
                collision_start_idx:
            ]
            ...
            for k_i in self.collision_buffer_additional_dict[s_i].keys():
                ...
                prepared_collision_infos[k_i] += self.collision_buffer_additional_dict[
                    s_i
                ][k_i][collision_start_idx:]
        ...
        collision_action = prepared_collision_scenario_actions
            ...
            if not collision_action:
                ...
                batch_info[k_i] = np.concatenate(
                    [
                        ...
                        np.stack(prepared_collision_infos[k_i])[
                            collision_sample_index - 1
                        ],
                    ]
                )
