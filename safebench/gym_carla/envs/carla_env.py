...
class CarlaEnv(gym.Env):
    ...
    def __init__(
        self, env_params, birdeye_render=None, display=None, world=None, logger=None
    ):
        ...
        # pixel_bin: spatial resolution (meters per pixel) of birdeye / camera observation
        self.pixel_bin = env_params["pixel_bin"]
        ...
        if self.scenario_category in ["planning", "scenic"]:
            self.obs_size = int(self.obs_range / self.pixel_bin)
    ...
    def reset(self, config, env_id, scenario_init_action):
        ...
        self.routeplanner = RoutePlanner(
            self.ego_vehicle, self.max_waypt, self.route_waypoints
        )
        (
            self.waypoints,
            _,
